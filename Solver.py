...
class ViticultureSolver:
    ...
    def NeedIsFulfilled(self, Order, Crush):
        ...
        for Bottle in BlushWines:
            BlushSolutions.append([Bottle, self.GenerateBlushSolutions(Bottle, CrushMap)])

        self.SolveBlushWines(BlushSolutions)
    ...
    def GenerateBlushSolutions(self, WineBottle, CrushMap):
    ...
    def SolveBlushWines(self, BlushSolutions):
            ...
            if CopySol:
                ...
                for Attempt in CopySol:
                    ...
                    Test = [x for x in CopySol if
                            x[GrapeType.RED] != Attempt[GrapeType.RED]
                            and
                            x[GrapeType.WHITE] != Attempt[GrapeType.WHITE]]
                    if Test:
                        Found = True
                        Choice = Attempt
                        break
            ...
            for j, SecItem in enumerate(BlushSolutions):
                ...
                if j > i:
                    BlushSolutions[j][1] = [x for x in SecSol if
                                            x[GrapeType.RED] != Choice[GrapeType.RED]
                                            and x[GrapeType.WHITE] != Choice[GrapeType.WHITE]]


    def DetermineBestPath(self, Order, CPad, FieldMap, FullPath, CurrentBranch=0, Years=0, CurrentPath=list(), Initial=True):
        """
        ...
        if self.NeedIsFulfilled(Order, CPad):
        ...
        if Initial:
            self.DetermineBestPath(Order, copy.deepcopy(CPad), FieldMap, FullPath, FieldType.MEDIUM, Years + 1,
                              copy.deepcopy(CurrentPath), False)
            self.DetermineBestPath(Order, copy.deepcopy(CPad), FieldMap, FullPath, FieldType.LARGE, Years + 1,
                              copy.deepcopy(CurrentPath), False)
            return
    ...
    def RemoveSuboptimalPaths(self, HarvestPaths):
    ...
    def DetermineCriticalHarvests(self, HarvestPaths, Order):
        ...
        for Path in HarvestPaths:
            ...
            for i in range(len(Path)):
                ...
                for Year in TestPath:
                    ...
                    Grapes = self.FieldMap[Year].HarvestField(True)
                    CPad.AddGrapes(Grapes)
                    CPad.AgeCrushPad()
                ...
                CriticalHarvestsInYear.append(not self.NeedIsFulfilled(Order, CPad))
    ...
    def DetermineNeededHarvests(self, HarvestPaths, Order):
    ...
    def ChoosePath(self, CriticalHarvests):
    ...
    def EstimateYears(self, Order, CPad, Cellar, FieldMap):

        NumNeeded = Order.GetNumRedWhiteGrapes()
        print("Number of red grapes needed: {0:d}".format(NumNeeded[GrapeType.RED]))
        print("Number of white grapes needed: {0:d}".format(NumNeeded[GrapeType.WHITE]))

        HarvestPaths = []
        self.DetermineBestPath(Order, CPad, FieldMap, HarvestPaths)
        HarvestPaths = self.RemoveSuboptimalPaths(HarvestPaths)
        CriticalHarvests = self.DetermineNeededHarvests(HarvestPaths, Order)
        self.DetermineCriticalHarvests(HarvestPaths, Order)
        CurrentPath = self.ChoosePath(CriticalHarvests)
